...
builder_info() {
    ...
    DEB_VERSION=${VERSION}${RELEASE_POST}
    DEB_FILE="${PKG_NAME}-${DEB_VERSION}.deb"
    ...
    verbose "Pkg version:  $DEB_VERSION"
...
build_data() {
    ...
    cp "$SRC_DIR/megahelp-silence"          "$DIST_DIR/data/usr/sbin/megahelp-silence"
    cp "$SRC_DIR/megahelp-report"           "$DIST_DIR/data/usr/sbin/megahelp-report"
    chmod -w                                "$DIST_DIR/data/usr/sbin"
    mkdir -p                                "$DIST_DIR/data/usr/share/doc/megahelp"
    cp "$SRC_DIR/README.md"                 "$DIST_DIR/data/usr/share/doc/megahelp/README.md"
    envsubst < "$TMPL_DIR/copyright"  >     "$DIST_DIR/data/usr/share/doc/megahelp/copyright" 
    cat "$SRC_DIR/LICENSE"           >>     "$DIST_DIR/data/usr/share/doc/megahelp/copyright"
    mkdir -p                                "$DIST_DIR/data/usr/share/man/man1"
    cp "$SRC_DIR/man.megahelp-silence.1"    "$DIST_DIR/data/usr/share/man/man1/megahelp-silence.1"
    cp "$SRC_DIR/man.megahelp-report.1"     "$DIST_DIR/data/usr/share/man/man1/megahelp-report.1"
    gzip -f                                 "$DIST_DIR/data/usr/share/man/man1/megahelp-silence.1"
    gzip -f                                 "$DIST_DIR/data/usr/share/man/man1/megahelp-report.1"
    mkdir -p                                "$DIST_DIR/data/etc/bash_completion.d"
    cp "$SRC_DIR/auto_megahelp.bash"        "$DIST_DIR/data/etc/bash_completion.d/megahelp"
